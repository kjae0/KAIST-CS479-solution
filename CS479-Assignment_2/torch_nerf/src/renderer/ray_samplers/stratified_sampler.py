...
class StratifiedSampler(RaySamplerBase):
    ...
    def sample_along_rays_uniform(
        ...
        z_vals = self.create_t_bins(num_sample, ray_bundle.origins.device)
        z_vals = z_vals.unsqueeze(0).expand(ray_bundle.origins.shape[0], -1)

        perturb = 1 / num_sample * torch.rand_like(z_vals)
        z_vals = z_vals + perturb
        z_vals = torch.clamp(z_vals, 0, 1)
        
        z_vals = self.map_t_to_euclidean(z_vals, ray_bundle.nears[0].item(), ray_bundle.fars[0].item())
        
        # mids = (z_vals[...,1:] + z_vals[...,:-1]) / 2
        # uppers = torch.concat([mids, z_vals[...,-1:]], -1)
        # lowers = torch.concat([z_vals[...,:1], mids], -1)
        # t_rand = torch.rand(z_vals.shape, device=z_vals.device) * 0.
