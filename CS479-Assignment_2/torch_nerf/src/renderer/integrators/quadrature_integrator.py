...
class QuadratureIntegrator(IntegratorBase):
    ...
    def integrate_along_rays(
        ...
        # sigma -> n_samples
        # radiance -> n_samples x 3
        # delta -> n_samples

        sigma = torch.nn.functional.relu(sigma)
        radiance = torch.nn.functional.sigmoid(radiance)

        sd_prod = (-1) * sigma * delta
        Ts = torch.exp(torch.cumsum(torch.cat((torch.zeros([sd_prod.shape[0], 1], device=sd_prod.device), sd_prod), -1)[:, :-1], -1))
        alpha = 1 - torch.exp(sd_prod)
        weights = Ts * alpha + 1e-10
        rgbs = torch.sum(weights.unsqueeze(-1) * radiance, -2)
